...
def handle_stats_command(stats_manager, chat_id: int, hours: int = 24):
    """
    Manipula o comando /stats para exibir estatísticas de banda.
    
    Args:
        stats_manager: Instância do StatisticsManager
        chat_id: ID do chat do Telegram
        hours: Número de horas para as estatísticas (padrão: 24)
    """
    try:
...
def handle_history_command(stats_manager, chat_id: int, days: int = 7):
    """
    Manipula o comando /history para exibir histórico de downloads.
    
    Args:
        stats_manager: Instância do StatisticsManager
        chat_id: ID do chat do Telegram
        days: Número de dias para o histórico (padrão: 7)
    """
    try:
        if not stats_manager:
            send_telegram("❌ Sistema de estatísticas não disponível.", chat_id, use_keyboard=True)
...
def handle_graph_command(stats_manager, chat_id: int, hours: int = 24):
    """
    Manipula o comando /graph para exibir gráfico de atividade.
    
    Args:
        stats_manager: Instância do StatisticsManager
        chat_id: ID do chat do Telegram
        hours: Número de horas para o gráfico (padrão: 24)
    """
    try:
        if not stats_manager:
            send_telegram("❌ Sistema de estatísticas não disponível.", chat_id, use_keyboard=True)
...
def handle_sync_command(sync_manager, chat_id: int):
    """
    Manipula o comando /sync para sincronizar manualmente com Jellyfin.
    
    Args:
        sync_manager: Instância do SyncManager
        chat_id: ID do chat do Telegram
    """
    try:
        if not sync_manager:
...
def handle_sync_status_command(sync_manager, chat_id: int):
    """
    Manipula o comando /sync_status para ver status da sincronização.
...
def handle_priority_command(sess, qb_url: str, chat_id: int, torrent_hash: str = None, priority: str = None):
    """
    Manipula o comando /priority para alterar prioridade de torrents.
    
    Args:
        sess: Sessão autenticada do qBittorrent
        qb_url: URL base da API do qBittorrent
        chat_id: ID do chat do Telegram
        torrent_hash: Hash do torrent (opcional)
        priority: Prioridade (increase, decrease, top, bottom)
    """
    try:
        if not sess:
...
💡 <b>Dica:</b> Use /qtorrents para ver os hashes dos torrents"""
        ...
        from qbittorrent_api import set_torrent_priority, get_torrent_info
        ...
        if success:
            ...
            send_telegram(
                ...
                f"📦 {name}\n"
                f"⚡ Nova prioridade: <code>{priority}</code>",
                chat_id,
                parse_mode="HTML",
                use_keyboard=True
            )
...
def handle_remove_command(sess, qb_url: str, chat_id: int, torrent_hash: str = None, delete_files: bool = False):
    """
    Manipula o comando /remove para remover torrents.
    ...
    Args:
        ...
        chat_id: ID do chat do Telegram
        torrent_hash: Hash do torrent (opcional)
        delete_files: Se True, remove também os arquivos
    """
    try:
        if not sess:
            send_telegram("❌ Não conectado ao qBittorrent.", chat_id, use_keyboard=True)
...
💡 <b>Dica:</b> Use /qtorrents para ver os hashes dos torrents"""
        ...
        from qbittorrent_api import delete_torrent, get_torrent_info
