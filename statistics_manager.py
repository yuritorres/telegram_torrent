...
class StatisticsManager:
    ...
    def __init__(self, qb_session, qb_url: str, data_file: str = "stats_data.json"):
        """
        Args:
            qb_session: Sessão autenticada do qBittorrent
            qb_url: URL base da API do qBittorrent
            data_file: Arquivo para armazenar dados históricos
        """
    ...
    def record_bandwidth(self):
        """Registra o uso atual de banda"""
        from qbittorrent_api import get_transfer_info
    ...
    def record_download(self, torrent_hash: str, name: str, size: int, category: str = None):
        """Registra um download concluído"""
        try:
            record = {
                'timestamp': datetime.now().isoformat(),
                'hash': torrent_hash,
    ...
    def get_bandwidth_stats(self, hours: int = 24) -> Dict:
        """Obtém estatísticas de banda para um período"""
        try:
            cutoff_time = datetime.now() - timedelta(hours=hours)
            
            recent_records = [
                r for r in self.bandwidth_history
            ...
            total_dl = sum(r['dl_data'] for r in recent_records)
            total_up = sum(r['up_data'] for r in recent_records)
            ...
            return {
                ...
                'avg_dl_speed': avg_dl_speed,
                'avg_up_speed': avg_up_speed,
                'peak_dl_speed': peak_dl_speed,
                'peak_up_speed': peak_up_speed,
    ...
    def get_download_history(self, days: int = 7) -> List[Dict]:
        """Obtém histórico de downloads para um período"""
        try:
    ...
    def get_activity_summary(self) -> Dict:
        """Obtém resumo de atividade geral"""
        try:
    ...
    def format_bandwidth_stats(self, hours: int = 24) -> str:
        """Formata estatísticas de banda para exibição"""
        from qbittorrent_api import format_bytes
    ...
    def format_download_history(self, days: int = 7) -> str:
        """Formata histórico de downloads para exibição"""
        from qbittorrent_api import format_bytes
    ...
    def format_activity_graph(self, hours: int = 24) -> str:
        """Gera um gráfico ASCII simples de atividade"""
        try:
            cutoff_time = datetime.now() - timedelta(hours=hours)
            ...
            for i in range(intervals):
                if buckets[i]['count'] > 0:
                    avg_dl = buckets[i]['dl'] / buckets[i]['count']
                    avg_speeds.append(avg_dl)
                else:
                    avg_speeds.append(0)
            ...
            for i, speed in enumerate(reversed(avg_speeds)):
                if max_speed > 0:
                    bar_level = int((speed / max_speed) * (len(bar_chars) - 1))
